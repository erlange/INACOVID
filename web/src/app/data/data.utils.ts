...
export function getDataCat(data: string): Model.IDataCategory[] {
  ...
  // console.log ('kondisi_penyerta', result.filter(m => m.Category === 'kondisi_penyerta'));
  // console.log ('gejala', result.filter(m => m.Category === 'gejala'));
  // console.log ('kelompok_umur', result.filter(m => m.Category === 'kelompok_umur'));
  return result;
}
