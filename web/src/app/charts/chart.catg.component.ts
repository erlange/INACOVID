...
export class ChartCatgComponent implements OnChanges, OnDestroy {
  ...
  setChartTypeOptions(type: string, lang: string): IChartTypeOptions {
    ...
    opts.series = [{
      ...
      type: 'bar',
      data: kond.map(n => n.Confirmed).map(n => parseFloat(n.toFixed(2))).sort((a, b) => a - b)
    },
    {
      name: i18n.ACTIVE[lang],
      type: 'bar',
      data: kond.map(n => n.Active).map(n => parseFloat(n.toFixed(2))).sort((a, b) => a - b)
    },
    {
      name: i18n.RECOVERED[lang],
      type: 'bar',
      data: kond.map(n => n.Cured).map(n => parseFloat(n.toFixed(2))).sort((a, b) => a - b)
    },
    {
      name: i18n.DEATHS[lang],
      type: 'bar',
      data: kond.map(n => n.Dead).map(n => parseFloat(n.toFixed(2))).sort((a, b) => a - b)
    },
  ];
