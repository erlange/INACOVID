...
export class MapComponent implements OnDestroy, OnInit, OnChanges {
  ...
  onHeaderClick(): void {
    this.HeaderClick.emit(this.CurrentLoc);
  }
  onMapToggle(show: boolean): void{
    if (show){
      L.tileLayer('http://a.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(this.map);
      // this.map.options.layers = [
      //   L.tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 18, attribution: '© OpenStreetMap contributors' })
      //   // this.map.addLayer(new L.Layer({attribution:'http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', }));
      // ];
    }
    else {
      L.tileLayer('http://a.tile.openstreetmap.org/{z}/{x}/{y}.png').remove();
      // this.map.options.layers = [
      //   L.tileLayer('', { maxZoom: 18, attribution: '...' })
      // ];
    }
